...
- React, TypeScript, Vite, Tailwind CSS, shadcn/ui (on Radix UI), ESLint, Prettier, React Router DOM and Zustand (global state — auth only, for now).
...
- `src/components/ui/shadcn`: components generated by the shadcn CLI and then **forked** to carry the BiotaGeom design. Excluded from the mutation and changed-line coverage gates, because their content is mostly class strings that mutation testing can only produce noise on.
...
- `src/hooks`, `src/types`, `src/utils` and `src/constants`: shared code only when there is real reuse. `src/utils/cn.ts` re-exports the `cn` class merger that every shadcn component imports.
...
- shadcn/ui is approved and configured (see `components.json`). Add components with `npm run ui:add -- <component>` — never `npx shadcn add` directly, since the wrapper rewrites the generated `cn` import to `@/utils/cn`.
- The components in `src/components/ui/shadcn` are forks, not pristine generator output: the stock variants were replaced with the product's own styling. Re-running `ui:add` for a component that already exists **discards that work** — use `--diff` to inspect before overwriting.
- Styling belongs in the component's `cva` variants, not at the call site. `cn()` runs tailwind-merge, so a `className` that conflicts with a variant class silently drops one of the two; passing a one-off override is how a design drifts. Add or adjust a variant instead.
- `Button`, `InputGroup` and the card parts require an explicit `variant` — there is no default. These styles are page-specific, so a silent fallback would hide a mistake at the call site rather than surface it at the type level.
- Never reintroduce shadcn's stock base classes on a fork. They set `h-8`, `rounded-lg`, `px-2.5`, `font-medium` and `[&_svg:not([class*='size-'])]:size-4` — that last one silently shrinks every icon in `ui/icons.tsx`, which sizes itself with `width`/`height` attributes rather than classes.
- shadcn's semantic tokens (`--primary`, `--background`, `--border`, ...) are bound to the BiotaGeom palette in `src/styles/global.css`. Change a colour there once, not per component.
- Code copied from the shadcn docs or third-party blocks imports from `@/components/ui/<name>`; adjust it to `@/components/ui/shadcn/<name>`.
